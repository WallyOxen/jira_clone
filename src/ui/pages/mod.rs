...
impl Page for EpicDetail {
  ...
  fn handle_input(&self, input: &str) -> Result<Option<Action>> {
      todo!() // match against the user input and return the corresponding action. If the user inpupt was invalid return None.
  }
}
...
impl Page for StoryDetail {
  fn draw_page(&self) -> Result<()> {
      let db_state = self.db.read_db()?;
      let story = db_state.stories.get(&self.story_id).ok_or_else(|| anyhow!("cound not find story!"))?;
...
mod tests {
  ...
  mod story_detail_page {
    ...
    fn handle_input_should_return_the_correct_actions() {
      ...
      let some_number = "1";
      let junk_input = "j983f2j";
      let junk_input_with_valid_prefix = "p986f2j";
      let input_with_trailing_white_space = "p\n";

      assert_eq!(page.handle_input(p).unwrap(), Some(Action::NavigateToPreviousPage));
      assert_eq!(page.handle_input(u).unwrap(), Some(Action::UpdateStoryStatus { story_id: story_id }));

  ...
  pub fn handle_action(&mut self, action: Action) -> Result<()> {
    match action {
      ...
      Action::NavigateToPreviousPage => {
        todo!() // remove the last page form the pages vector
      },
      Action::CreateEpic => {
      ...
      Action::UpdateStoryStatus { story_id } => {
        todo!() // prompt the user to update status and persist it in the databse
      },
...
mod tests {
  ...
  fn handle_action_should_handle_create_epic() {
    ...
    assert_eq!(epic.description, "desecription".to_owned())    
  }
  ...
  fn handle_action_should_handle_update_epic() {
    ...
    nav.handle_action(Action::UpdateEpicStatus { epic_id: epic_id }).unwrap();
  ...
  #[test]
  fn handle_action_shuold_handle_delete_epic() {
    let db = Rc::new(JiraDatabase { database: Box::new(MockDB::new()) });
    let epic_id = db.create_epic(Epic::new("".to_owned(), "".to_owned())).unwrap();
    ...
    nav.handle_action(Action::DeleteEpic { epic_id: epic_id }).unwrap();
  ...
  #[test]
  fn handle_action_should_handle_story_update() {
    let db = Rc::new(JiraDatabase { database: Box::new(MockDB::new()) });
    let epic_id = db.create_epic(Epic::new("".to_owned(), "".to_owned())).unwrap();
    ...
    nav.handle_action(Action::UpdateStoryStatus { story_id: story_id }).unwrap();
  ...
  fn handle_action_should_handle_delete_story() {
    ...
    nav.handle_action(Action::DeleteStory { epic_id: epic_id, story_id: story_id }).unwrap();
